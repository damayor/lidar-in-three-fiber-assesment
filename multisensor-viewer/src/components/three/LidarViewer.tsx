...
import { OrbitControls, Grid, Stats, GizmoHelper, GizmoViewport, Html } from "@react-three/drei";
import { PointCloud } from "./PointCloud";
import { AnnotationBoxes } from "./AnnotationBoxes";
import type { Annotation, Point3D, TransformData } from "../../interfaces/types";
...
export const LidarViewer: FC<LidarViewerProps> = ({ points, annotations, highlightedToken, transformData }) => {
  ...
  return (
    <div className="relative h-120 bg-[#050810]">
      <Canvas
        camera={{ position: [0, 20, 15], fov: 60, near: 0.1, far: 1000 }}
        ...
        <PointCloud points={points} />
        {
          transformData ? (
            ...
            />
                ...
                <div style={{ background:"rgba(0,0,0,.9)", border:`1px solid red`, color:'#f00', fontSize:9, fontFamily:"monospace", padding:"3px 8px", borderRadius:2, whiteSpace:"nowrap", pointerEvents:"none" }}>
                  ANY ANNOTATION DATA PROVIDED
                </div>
              </Html>
        }
      ...
      <div className="absolute top-3 left-3 text-[10px] font-mono text-[#636e7b] pointer-events-none leading-relaxed">
        ...
        <span className="text-cyan-400">BOXES</span> · {annotations.length} objects
      </div>
